...
function readableTextColor([r, g, b]: [number, number, number]): "0;0;0" | "255;255;255" {
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", (_event, ctx) => {
		ctx.ui.setStatus("hex-color-history", "🎨 hex history");
		ctx.ui.notify("Hex color highlighting enabled in chat history", "info");
	});

	pi.registerCommand("hexcolor", {
		description: "Show a hex color highlighting test message in chat history",
		handler: async (_args, _ctx) => {
			pi.sendMessage({
				customType: "hex-color-test",
				content: "Hex color test: #ff0000 #00ff00 #3366cc #f0c #222 #eeeeee",
				display: true,
			});
		},
	});
